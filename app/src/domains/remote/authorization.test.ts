...
describe('remote command authorization', () => {
  ...
  it('allows switching to a detached workspace (the switch action reclaims it)', () => {
    const withDetached = { ...scoped, detachedWorkspaces: ['background'] } as AppState
    expect(remoteCommandAllowed(withDetached, command('workspace_switch', 'background'))).toBe(true)
