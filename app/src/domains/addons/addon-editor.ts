...
- methods (each needs its permission granted): getState, sessions.readOutput(id, lines), templates.list(), tasks.get(id) [state:read] · sendToSession(id, text), sessions.stop(id) [sessions:send] · launchSession(cmd, cwd, name), templates.run(idOrName, task) [sessions:launch] · focusSession(id), focusTask(taskId), flash(t), notify(title, detail), logEvent(t) [ui] · tasks.add(title, col, {description, criteria[], cwd, typeId, templateId, machineId, isolate, sessionMode, scheduleAt})/tasks.update(id, patch)/tasks.rename/tasks.move(id, col)/tasks.remove(id)/tasks.start(id)/tasks.restart(id)/tasks.chat(id, text)/tasks.approve(id)/tasks.reject(id, feedback) [tasks] (cols: backlog|progress|review|done|failed; started tasks get a watcher-driven one-shot by default; tasks.chat talks to the task's watcher; approve merges isolated worktrees) · schedules.add({name, schedule|at, cmd|task})/schedules.toggle(name, on)/schedules.remove(name) [schedules] · agent.wake(note) → reply [agent] (the addon's own LLM agent, declared via an "agent": {system, on: [hooks], every: cron} field) · storage.get(key)/storage.set(key, value)/storage.list()/storage.remove(key) [storage] · http.request(method, url, {headers, body}) → {status, contentType, text} [http] (host must be in the package's "hosts" allowlist, https only, redirects blocked; header/body values may embed {{secret:NAME}} — declared in the package's "secrets" and substituted from the OS keychain; the destination receives them and its response is visible to the addon) · secrets.list() → [{name, label, set}] [secrets]
