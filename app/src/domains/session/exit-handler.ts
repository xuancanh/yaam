...
// ./exit (classifyExit); this owns the effectful fan-out. Extracted from the
// provider into the session domain.
...
export function useSessionExitHandler(ctx: SessionExitCtx): void {
  useEffect(() => {
    const { stateRef, takeUserStopped, taskForSession, pushTaskChat, logEvent, notify, fireAddonHook, runWatcher, monitorEvent } = ctx
