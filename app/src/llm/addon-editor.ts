...
- methods (each needs its permission granted): getState [state:read] · sendToSession(id, text) [sessions:send] · launchSession(cmd, cwd, name) [sessions:launch] · focusSession(id), flash(t), notify(title, detail), logEvent(t) [ui] · tasks.add(title, col)/tasks.rename(id, title)/tasks.move(id, col)/tasks.remove(id)/tasks.start(id) [tasks] (cols: backlog|progress|review|done|failed; tasks.start spawns a session for the task) · storage.get(key)/storage.set(key, value) [storage]
