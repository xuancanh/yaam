...
- methods (each needs its permission granted): getState, sessions.readOutput(id, lines), templates.list() [state:read] · sendToSession(id, text), sessions.stop(id) [sessions:send] · launchSession(cmd, cwd, name), templates.run(idOrName, task) [sessions:launch] · focusSession(id), flash(t), notify(title, detail), logEvent(t) [ui] · tasks.add(title, col, {description, criteria[], cwd, typeId, templateId})/tasks.update(id, patch)/tasks.rename/tasks.move(id, col)/tasks.remove(id)/tasks.start(id)/tasks.restart(id)/tasks.chat(id, text) [tasks] (cols: backlog|progress|review|done|failed; started tasks get a watcher-driven one-shot; tasks.chat talks to the task's watcher) · schedules.add({name, schedule|at, cmd|task})/schedules.toggle(name, on)/schedules.remove(name) [schedules] · agent.wake(note) → reply [agent] (the addon's own LLM agent, declared via an "agent": {system, on: [hooks]} field) · storage.get(key)/storage.set(key, value) [storage]
