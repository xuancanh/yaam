...
function fakePort(over: Partial<SessionProcessPort> = {}): SessionProcessPort {
  return {
    ...
    writeSession: vi.fn(async () => {}), sendLine: vi.fn(), detectCliSession: vi.fn(async () => null), hooksInfo: vi.fn(async () => null), watchTranscript: vi.fn(async () => {}), unwatchTranscript: vi.fn(async () => {}), freePort: vi.fn(async () => null), watchOpencode: vi.fn(async () => {}), unwatchOpencode: vi.fn(async () => {}),
