...
| `fs` | Symlink-aware workspace containment and destructive-root guards are strong. Ranged reads, command output, and credential helpers were not actually resource-bounded. | **P1 verified:** streaming ranged reads, bounded-tail pipe draining, and a shared 30-second/1 MB credential runner with noisy-process/EOF/timeout tests. Next: cap directory results and text write sizes, reduce remaining TOCTOU windows. |
